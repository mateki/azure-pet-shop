...
public class PetStoreServiceImpl implements PetStoreService {
    ...
    public Collection<Product> getProducts(String category, List<Tag> tags) {
        ...
        try {
            ...
            logger.info("Log number of products: " + products.size());
            this.sessionUser.getTelemetryClient().getContext().getProperties().put("user_name",this.sessionUser.getName());
            this.sessionUser.getTelemetryClient().getContext().getProperties().put("session_id",this.sessionUser.getSessionId());
            this.sessionUser.getTelemetryClient().getContext().getProperties().put("products_size",""+products.size());
            this.sessionUser.getTelemetryClient().trackTrace(String.format("Telemetry SessionId: %s userName: %s number of products %s", this.sessionUser.getSessionId(), this.sessionUser.getName(), products.size()));
            return products;
        } catch (
